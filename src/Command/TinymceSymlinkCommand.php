...
/**
 * Command to check and create tinymce symlink into TinymceBundle.
 *
...
{
    public static $tinymceBundleName = 'gibilogic/tinymce-bundle';
    ...
    public static function checkSymlink($symlinkTarget, $symlinkName, $forceSymlink = false)
    {
        if ($forceSymlink && file_exists($symlinkName) && !is_link($symlinkName)) {
            if ('link' != filetype($symlinkName)) {
                throw new \Exception(sprintf('"%s" exists and is no link.', $symlinkName));
            }
            ...
            if ($linkTarget != $symlinkTarget) {
                if (!$forceSymlink) {
                    throw new \Exception(
                        sprintf(
                            'Symlink "%s" points to "%s" instead of "%s".',
                            $symlinkName,
                            $linkTarget,
                            $symlinkTarget
                        )
    ...
    protected function configure()
    {
        $this
            ...
            ->addArgument(
                'pathToStfalconTinymceBundle',
                InputArgument::OPTIONAL,
                'Where is StfalconTinymceBundle located?'
            )
            ...
            ->addOption(
                ...
                'If set please specify pathTo'.static::$pathName.', and pathToStfalconTinymceBundle'
            )
            ->addOption(
                'no-symlink',
                null,
                InputOption::VALUE_NONE,
                'Use hard copy/mirroring instead of symlink. This is required for Windows without administrator privileges.'
            )
...
EOT
    ...
    protected function getTinymcePathsFromUser()
    {
        ...
        if (!is_dir(dirname($symlinkName))) {
            throw new \Exception(sprintf('pathToStfalconTinymceBundle: "%s" does not exist', dirname($symlinkName)));
        }
        if (empty($symlinkTarget)) {
            throw new \Exception(sprintf('"%s" not specified', static::$pathName));
        }
        if (substr($symlinkTarget, 0, 1) == '/') {
            ...
            if (!is_dir($symlinkTarget)) {
                throw new \Exception(sprintf('Target path "%s" is not a directory.', $symlinkTarget));
            }
        } else {
        ...
        if (!is_dir($symlinkTarget)) {
            throw new \Exception(
                sprintf(
                    "\"%s\" would resolve to: \"%s\"\n and this is not reachable from \npathToStfalconTinymceBundle: \"%s\"",
                    static::$pathName,
                    $symlinkTarget,
                    dirname($symlinkName)
                )
